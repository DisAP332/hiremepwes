...
const MAX_PENDING_BIDS_PER_PHONE = 3;
...
export async function POST(request: Request) {
  try {
    ...
    const bid = await prisma.$transaction(
      async (tx) => {
        const [dailySessionCount, conflictingEvent, existingBid, pendingBidCount] =
          await Promise.all([
        ...
        if (pendingBidCount >= MAX_PENDING_BIDS_PER_PHONE) {
          throw new Error("PHONE_BID_LIMIT");
        }
    ...
    if (error instanceof Error) {
      const messages: Record<string, string> = {
        ...
        PHONE_BID_LIMIT:
          "This phone number already has three pending bids. Please wait for a response before adding another.",
      };
