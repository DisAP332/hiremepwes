...
function findService(category?: string | null) {
  return (
    serviceCards.find((service) => service.category === category) ??
    serviceCards[0]
  );
}
...
export function BookingWizard() {
  ...
  const { step, selectedCategory, uploadedPhotoUrls } = useAppSelector(
    (state) => state.bookingWizard,
  );
  const startingService = useMemo(
    () => findService(params.get("service")),
    [params],
  );
  ...
  const form = useForm<BookingRequestInput>({
    resolver: zodResolver(
      bookingRequestSchema,
    ) as Resolver<BookingRequestInput>,
    defaultValues: {
  ...
  useEffect(() => {
    ...
    form.setValue("serviceCategory", startingService.category, {
      shouldValidate: true,
    });
    form.setValue("serviceLabel", startingService.title, {
      shouldValidate: true,
    });
  ...
  const updateSocial = (platform: SocialPlatform, handle: string) => {
    ...
    form.setValue("socialLink", buildSocialValue(platform, handle), {
      shouldValidate: true,
    });
  };
  ...
  const submit = async (values: BookingRequestInput) => {
    ...
    const finalValues = {
      ...values,
      socialLink:
        buildSocialValue(socialPlatform, socialHandle) || values.socialLink,
    };
    ...
    <section className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      <div className="mb-4 rounded-[1.5rem] border border-white/70 bg-white/75 p-4 shadow-xl shadow-lavender-200/20 backdrop-blur-xl sm:mb-6 sm:rounded-[2rem]">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-raspberry-300">
              Request form
            </p>
          ...
          <div className="rounded-full bg-lemon-100 px-3 py-1 text-xs font-black text-ink">
            {step + 1}/{chapters.length}
          </div>
        </div>
        <div className="mt-4 h-3 rounded-full bg-white/80 p-1">
          <div
            className="h-full rounded-full bg-gradient-to-r from-raspberry-300 via-coral-300 to-aqua-300 transition-all"
      ...
      <form
        ...
        className="card-pop p-4 sm:p-6 lg:p-8"
      >
        {step === 0 ? (
          <motion.div
          ...
          >
            <p className="text-sm font-black uppercase tracking-[0.24em] text-raspberry-300">
              Choose a service
            </p>
            ...
            <p className="mt-3 text-sm leading-6 text-ink/65 sm:text-base">
              Pick one card. The next step changes to match your choice.
            </p>
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {serviceCards.map((service) => {
                const selected =
                  selectedCategory === service.category ||
                  chosenCategory === service.category;
                return (
                  <button
                  ...
                  >
                    <div className="h-full rounded-[1.55rem] bg-white/90 p-4 sm:p-5">
                      <div className="flex items-center gap-3">
                        <div
                          className={`grid size-14 place-items-center rounded-[1.25rem] bg-gradient-to-br ${service.accent} text-3xl`}
                        >
                          {service.emoji}
                        </div>
                        <div>
                          ...
                          <p className="text-xs font-black text-raspberry-300">
                            {service.startingAt}
                          </p>
                        </div>
                      </div>
                      ...
                      {selected ? (
                        <p className="mt-3 rounded-full bg-lemon-100 px-3 py-2 text-center text-xs font-black text-ink">
                          Selected ✨
                        </p>
                      ) : null}
                    </div>
        ...
        {step === 1 ? (
          ...
          >
            <div
              className={`rounded-[1.75rem] bg-gradient-to-br ${chosenService.accent} p-1`}
            >
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-ink sm:text-3xl">
                      {chosenService.shortTitle}
                    </h2>
                    <p className="text-sm font-bold text-ink/60">
                      {chosenService.questPrompt}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <>
                  <label>
                    ...
                    <input
                      ...
                      placeholder="Kitchen + bath, one bedroom, whole apartment..."
                      {...form.register("homeSize")}
                    />
                  </label>
                  <label>
                    <span className="label-cute">
                      Mess intensity: {form.watch("intensity") ?? 3}/5
                    </span>
                    <input
                      type="range"
                      min="1"
                  ...
                  <label>
                    ...
                    <select
                      className="input-cute"
                      {...form.register("hasSupplies")}
                    >
                      <option value="yes">
                        Yes, I have supplies you can use
                      </option>
                  ...
                  <label>
                    ...
                    <input
                      className="input-cute"
                      placeholder="Vacuum, mop, sprays, trash bags..."
                      {...form.register("suppliesNote")}
                    />
                  </label>
                </>
              ) : null}
              ...
              {chosenCategory === "pc_phone_repair" ? (
                <label className="sm:col-span-2">
                  ...
                  <input
                    className="input-cute"
                    placeholder="Windows PC, iPhone, Android, printer, Wi‑Fi, slow laptop..."
              ...
              {chosenCategory === "ai_data_protection" ? (
                <label className="sm:col-span-2">
                  ...
                  <input
                    className="input-cute"
                    placeholder="AI app privacy, account permissions, metadata, scams, safer sharing..."
                    {...form.register("aiPrivacyConcern")}
                  />
                </label>
              ) : null}
              ...
              {chosenCategory === "masks_crafts" ? (
                <div className="sm:col-span-2 rounded-[1.5rem] bg-lavender-100/70 p-4 text-sm leading-6 text-ink/70">
                  Tell me the mask/craft idea in the notes below. Useful clues:
                  colors, size, vibe, deadline, reference images, and budget
                  range.
                </div>
              ) : null}
              ...
              <label className="sm:col-span-2">
                <span className="label-cute">
                  What would make this go well?
                </span>
                <textarea
                  className="input-cute min-h-32"
                  placeholder="A few practical details are perfect. No shame, no essay required."
                  {...form.register("notes")}
                />
              </label>
              ...
              <div className="sm:col-span-2 rounded-[1.5rem] border border-dashed border-raspberry-200 bg-raspberry-200/10 p-4 sm:p-5">
                <label className="flex cursor-pointer flex-col items-center justify-center gap-3 text-center">
                  ...
                  <span className="max-w-md text-sm leading-6 text-ink/60">
                    Photos help with estimates, but they are not required if you
                    feel embarrassed or prefer privacy.
                  </span>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {uploadedPhotoUrls.map((url) => (
                      <button
                        key={url}
                        type="button"
                        onClick={() => dispatch(removeUploadedPhotoUrl(url))}
          ...
          >
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="sm:col-span-2">
                ...
                <textarea
                  className="input-cute min-h-28"
                  placeholder="Example: Any weekday after noon, this Saturday, mornings preferred..."
                  {...form.register("preferredDates")}
                />
                <ErrorText
                  message={form.formState.errors.preferredDates?.message}
                />
              </label>
              <label>
              ...
              <label>
                ...
                <input
                  className="input-cute"
                  {...form.register("budgetPreference")}
                />
              </label>
              <label className="sm:col-span-2">
                <span className="label-cute">
                ...
                <input
                  className="input-cute"
                  placeholder="Parking, pets, quiet hours, preferred contact times..."
                  ...
                  <input
                    ...
                    placeholder={
                      socialPlatform === "other"
                        ? "handle, name, or profile link"
                        : "your handle"
                    }
                    value={socialHandle}
                    onChange={(event) =>
                      updateSocial(socialPlatform, event.target.value)
                    }
                  />
                </div>
                <input type="hidden" {...form.register("socialLink")} />
                <p className="mt-2 text-xs font-bold text-ink/50">
                  Just a handle is okay. This is only for basic safety
                  verification.
                </p>
                <ErrorText
                  message={form.formState.errors.socialLink?.message}
                />
              </div>
              <label className="sm:col-span-2">
                <span className="label-cute">
                  Address details, optional until confirmed
                </span>
                <input
                  className="input-cute"
                  placeholder="Neighborhood, cross streets, apartment access notes..."
        ...
        {step === 4 ? (
          ...
          >
            ...
            <div className="mt-6 space-y-3 rounded-[1.5rem] bg-lemon-100 p-4 text-sm leading-6 text-ink/75 sm:p-5">
              ...
              <p>
                <strong>Minimum:</strong> $100 / $30 hourly unless quoted
                differently.
              </p>
              <p>
              ...
              <p>
                <strong>Privacy:</strong> Your form info is only used to respond
                to your request and prepare the service.
              </p>
              {uploadedPhotoUrls.length ? (
                <p>
                  <strong>Photos:</strong> {uploadedPhotoUrls.length} uploaded
                </p>
              ) : (
                <p>
                  <strong>Photos:</strong> none, which is totally okay.
                </p>
              )}
            </div>
            {Object.keys(form.formState.errors).length ? (
              <div className="mt-5 rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-700">
                Some required fields need attention before sending. Use the step
                buttons above to jump back.
              </div>
            ) : null}
          </motion.div>
        ) : null}
        ...
        <div className="mt-7 border-t border-ink/10 pt-5">
          {serverMessage ? (
            <p
              className={`mb-4 rounded-2xl bg-white/75 p-3 text-sm font-bold ${status === "error" ? "text-red-600" : status === "success" ? "text-green-700" : "text-ink/60"}`}
            >
              {serverMessage}
            </p>
          ) : null}
          <div className="grid grid-cols-2 gap-3 sm:flex sm:items-center sm:justify-between">
            <button
              ...
              onClick={back}
              disabled={step === 0}
              className="btn-secondary px-4 py-3 disabled:cursor-not-allowed disabled:opacity-40"
            >
            ...
            {step < chapters.length - 1 ? (
              ...
              <button
                ...
                className="btn-primary px-4 py-3 disabled:cursor-wait disabled:opacity-60"
              >
