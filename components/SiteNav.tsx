  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/50 bg-white/72 backdrop-blur-xl">
      <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between gap-2 px-3 sm:h-20 sm:px-6 lg:px-8">
        <Link href="/" aria-label="JB Services home" className="group flex min-w-0 items-center gap-2">
          <span className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full border border-white/70 bg-white/70 shadow-lg shadow-pink-200/40 transition group-hover:scale-105 sm:h-12 sm:w-12">
            <img src={bellLogo} alt="JB Services bell logo" className="h-full w-full object-contain p-1" />
          </span>
          <span className="jb-rainbow-logo text-[1.05rem] font-black uppercase tracking-[0.12em] sm:text-2xl">JB Services</span>
        </Link>
      ...
      <style>{`
        .jb-rainbow-logo {
          ...
          animation: jb-rainbow-shift 5s ease-in-out infinite;
          white-space: nowrap;
        }
        @keyframes jb-rainbow-shift {
